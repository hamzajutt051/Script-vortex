...
export default function MainPage() {
  ...
  const circle = (doc) => {
    ...
    const offsetFromEdge = 12.5;
    const circleRadiusPoints = 2.5;

    doc.setFillColor(0, 0, 0);
    doc.circle(offsetFromEdge, offsetFromEdge, circleRadiusPoints, "F");
    doc.circle(
      pageWidth - offsetFromEdge,
      offsetFromEdge,
      circleRadiusPoints,
      "F"
    ); // Top-right corner
    ...
    images.forEach((img, index) => {
      html2canvas(document.getElementById(`image-${index}`)).then((canvas) => {
        ...
        if (index % 3 === 0 && index !== 0) {
          xPosition = 8.6;
          yPosition += imageHeight + 26.6;
        } else if (index !== 0) {
          xPosition += imageWidth + 14.0;
        }
        ...
        // Check if it's the last image before saving
        if (index === images.length - 1) {
          doc.save("images-9-.pdf");
        }
      });
    });
  };
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              for (let i = 0; i < files.length; i++) {
                if (images.length + imgs.length < 9) {
